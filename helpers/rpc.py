...
class BaseProxy2(object):
    """
    Base JSON-RPC proxy class "without https" support 
    User input test host IP address (where bitcoind is running)
    and listening RPC port (rpcport=10911 in bitcoin.conf)
    
    TODO: 
    we want to access the remote test host .bitcoin.conf file 
    to obtain the settings (e.g. rpcpassword, rpcuser for authpair which
    is used in connection header)
    
    For now, just make sure this same bitcoin.conf file is also in the
    .bitcoin folder of the localhost running the BUTool
    """
    print("Calling Local BaseProxy2 .....\n")
    hostname = None
    hostport = None
    def __init__(self,
                 service_url=None,
...
#        print("self.__url = ", self.__url)
        ...
        self.__id_count = 0
        authpair = "%s:%s" % (self.__url.username, self.__url.password)
        print("authpair = ", authpair)
        authpair = authpair.encode('utf8')
        self.__auth_header = b"Basic " + base64.b64encode(authpair)
        print("<3> hostname = ", hostname)

        if hostname:
            self.__conn = httplib.HTTPConnection(hostname, port=hostport,
                                             timeout=timeout)
...
class Proxy2(BaseProxy2):
    """Proxy to a bitcoin RPC service
    Uses BaseProxy2 to send data to another host with different IP (not localhost)
    """
    ...
    def __init__(self,
        ...
        The host url and port must specified as parameters.
        ``timeout`` - timeout in seconds before the HTTP interface times out
        
        TODO: config file is read from local machine, which is not correct. 
        Need to clean up at BaseProxy2 and possibly figure how to read in the 
        config file from the Remoted Machine !!!
        """

        super(Proxy2, self).__init__(service_url=service_url,
                                    service_port=service_port,
                                    btc_conf_file=btc_conf_file,
                                    timeout=timeout,
                                    **kwargs)
